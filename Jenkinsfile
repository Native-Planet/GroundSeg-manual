            ...
            steps {
                sh (
                    script: '''#!/bin/bash -x
                        ...
                        img_hash=`curl -s "https://hub.docker.com/v2/repositories/nativeplanet/groundseg-manual/tags/${channel}/?page_size=100" \
                            |jq -r '.images[]|select(.architecture=="amd64").digest'|sed 's/sha256://g'`
                        curl -X PUT -H "X-Api-Key: ${versionauth}" \
                            https://version.groundseg.app/modify/groundseg/${channel}/manual/amd64_sha256/${img_hash}
                        if [ "${channel}" = "latest" ]; then
                            curl -X PUT -H "X-Api-Key: ${versionauth}" \
                                https://version.groundseg.app/modify/groundseg/canary/manual/amd64_sha256/${img_hash}
                        fi
                    ''',
                    returnStdout: true
                )
